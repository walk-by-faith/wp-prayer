...
if (isset($shortcode_atts)) {
    if (array_key_exists('type', $shortcode_atts)) {
        ...
        if (array_key_exists('form', $shortcode_atts)) {
            $isForm = $shortcode_atts['form'];
        }
    } else {
        //do nothing
    }
...
$prayerType1 = __('Prayer Request', WPE_TEXT_DOMAIN);
if ($prayerType === 'praise') {
    $prayerType1 = __('Praise Report', WPE_TEXT_DOMAIN);
}
//making prayer type compatible with our below code
if ($prayerType === 'prayer')
    $prayerType = 'prayer_request';
elseif ($prayerType === 'praise')
    $prayerType = 'praise_report';
else // do nothing
